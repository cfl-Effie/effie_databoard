...
const state = {
  ...
  forecast: Array.from({ length: 12 }, (_, i) => 800 + Math.round(rand(-120, 220)) + i * 18),
...
function renderChannels() {
  channelChart.setOption({
    ...
    legend: { bottom: 0, textStyle: { color: '#6b86b3' }, itemWidth: 10, itemHeight: 10 },
    series: [{
      type: 'pie',
      radius: ['42%', '68%'],
      center: ['50%', '44%'],
...
function renderSummary() {
  ...
  const last6 = state.forecast.slice(-6);
  ...
  document.getElementById('summary').innerHTML =
    ...
    `模型预测未来 6 个时段订单量将<span class="hl">${trend >= 0 ? '持续走高' : '短暂回调'}</span>，请提前预备运力。<br>` +
...
function tickSlow() {
  ...
  const last = state.forecast[state.forecast.length - 1];
  state.forecast.push(Math.max(400, Math.round(last * rand(0.95, 1.12))));
