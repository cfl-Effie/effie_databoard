...
### 9. 数据中枢态势（拓扑关系图）
大屏 C 位的关系图：中心「数据中枢」节点辐射 6 个业务组件（采集网关 / 指标仓库 / 告警中心 / 日志服务 / 实时计算 / API 边缘），节点间带科技感虚线链路与箭头，悬停高亮邻接。

### 10. 平台能力雷达图
五维能力评估（吞吐 / 弹性 / 安全 / 延迟 / 稳定），填充面积代表综合能力，数值缓慢波动。

### 11. 服务节点状态条（底部通栏）
5 个服务实例的 `online` / `busy` 状态徽标 + 负载进度条，负载实时浮动、状态偶发切换，直观区分在线与繁忙节点。
...
- [x] 阶段二：大屏流式自适应（100% 铺满任意分辨率，图表随窗口 resize，不变形不留黑边）
